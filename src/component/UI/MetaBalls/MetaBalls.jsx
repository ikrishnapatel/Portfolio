...
const MetaBalls = ({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < effectiveBallCount; i++) {
      ...
      const dtFactor = 0.1 * Math.PI + h1[1] * (0.6 * Math.PI - 0.1 * Math.PI); // slightly more variable speed
      const baseScale = 8.0 + h1[1] * (35.0 - 8.0); // Much wider spread
      const h2 = hash33(h1);
      const toggle = Math.floor(h2[0] * 3.0); // more randomness
      const radiusVal = 0.5 + h2[2] * (2.5 - 0.5);
    ...
    function update(t) {
      ...
      for (let i = 0; i < effectiveBallCount; i++) {
        ...
        const posX = x * p.baseScale * clumpFactor * 2.2; // Stretch left/right
        const posY = y * p.baseScale * clumpFactor * 0.8; // Constrain top/bottom a bit
